...
export class TipoDocSapRepository {
  ...
  private get DB(): string {
    return tbl(this.schema, "REND_TIPO_DOC_SAP", this.dbType);
  }
  ...
  private norm(row: Record<string, any>): TipoDocSap {
  ...
  async exists(idTipo: number): Promise<boolean> {
    const rows = await this.db.query(
      `SELECT 1 FROM ${this.DB} WHERE "U_IdTipo" = ?`,
      [idTipo],
    );
  ...
  async update(
    ...
    if (dto.esTipoF !== undefined) {
      parts.push('"U_EsTipoF" = ?');
      params.push(dto.esTipoF);
    }
    if (dto.permiteGU !== undefined) {
      parts.push('"U_PermiteGU" = ?');
      params.push(dto.permiteGU ? "Y" : "N");
    }
    if (dto.permiteGD !== undefined) {
      parts.push('"U_PermiteGD" = ?');
      params.push(dto.permiteGD ? "Y" : "N");
    }
    if (dto.orden !== undefined) {
      parts.push('"U_Orden" = ?');
      params.push(dto.orden);
    }
  ...
  async remove(idTipo: number): Promise<{ affected: number }> {
    const affected = await this.db.execute(
      `DELETE FROM ${this.DB} WHERE "U_IdTipo" = ?`,
      [idTipo],
    );
    return { affected };
  }
