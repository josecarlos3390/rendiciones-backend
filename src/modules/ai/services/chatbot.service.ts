...
export class ChatbotService {
  ...
  async procesarConsulta(dto: ChatConsultaDto): Promise<ChatRespuesta> {
    if (!this.aiConfig.enabled) {
      return {
        mensaje:
          "Las funcionalidades de IA no están habilitadas. Contacta al administrador.",
        tipo: "texto",
    ...
    this.logger.log(
      `Chatbot: "${dto.mensaje}" - Modo: ${esOnline ? "ONLINE" : "OFFLINE"}`,
    );
    ...
    try {
      ...
      if (this.claudeService.esConsultaContable(dto.mensaje)) {
        this.logger.log(
          "Detectada consulta contable boliviana, usando prompt especializado",
        );
        const resultadoContable =
          await this.claudeService.consultarNormaContable(dto.mensaje);
      ...
      return {
        ...
        tipo: (respuestaIA.tipo as string as ChatRespuesta["tipo"]) || "texto",
        datos: respuestaIA.datos,
        sugerencias:
          respuestaIA.sugerencias || this.generarSugerencias(dto.mensaje),
        timestamp: new Date().toISOString(),
      };
    } catch (error: unknown) {
      this.logger.error(
        `Error chatbot: ${error instanceof Error ? error.message : String(error)}`,
      );
      return {
  ...
  private async recolectarContexto(
  ...
  private generarSugerencias(consulta: string): string[] {
    ...
    return (
      temas[
        Object.keys(temas).find((k) => consulta.toLowerCase().includes(k)) ||
          "default"
      ] || temas.default
    );
  }
  ...
  private generarSugerenciasContabilidad(consulta: string): string[] {
    const temas: Record<string, string[]> = {
      ...
      iva: [
        "Crédito fiscal IVA",
        "RC-IVA en alquileres",
        "Facturas sin crédito fiscal",
      ],
      retencion: [
    ...
    const clave = Object.keys(temas).find((k) =>
      consulta.toLowerCase().includes(k),
    );
  ...
  private obtenerRespuestaFallback(consulta: string): string {
    ...
    if (q.includes("gastado"))
      return "Este mes llevas Bs. 6,500 en 3 rendiciones.";
    if (q.includes("presupuesto"))
      return "Tu presupuesto es Bs. 10,000. Te quedan Bs. 3,500.";
