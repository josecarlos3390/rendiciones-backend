...
export class AdjuntosController {
  ...
  @ApiOperation({ summary: 'Listar adjuntos de una línea de rendición' })
  @ApiParam({ name: 'idRendicion', description: 'ID de la rendición' })
  @ApiParam({ name: 'idRD', description: 'ID de la línea de detalle' })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        file: {
          type: 'string',
          format: 'binary',
          description: 'Archivo (PDF, JPG, PNG, DOC, DOCX, XLS, XLSX) - Max 10MB',
        },
        descripcion: {
          type: 'string',
