...
import {
  IPrctjRepository,
  PRCTJ_REPOSITORY,
} from "./repositories/prctj.repository.interface";
import { RendDService } from "@modules/rend-d/rend-d.service";
import { RendMService } from "@modules/rend-m/rend-m.service";
import { SavePrctjDto } from "./dto/prctj.dto";
import { RendD } from "@modules/rend-d/interfaces/rend-d.interface";
...
export class PrctjService {
  ...
  constructor(
    ...
    private readonly repo: IPrctjRepository,
  ...
  async findByLinea(
    ...
    await this.rendDSvc.findByRendicion(
      idRendicion,
      role,
      idUsuario,
      loginUsername,
  ...
  async save(
    ...
    if (role !== "ADMIN") {
      ...
      const puedeEditar = esPropietario
        ? cabecera.U_Estado === EstadoRendicion.ABIERTO
        : esAprobador && cabecera.U_Estado === EstadoRendicion.ENVIADO;
      ...
      if (!esPropietario && esAprobador) {
        const esSub = await this.rendMSvc.isSubordinado(
          cabecera.U_IdUsuario,
          loginUsername,
        );
        if (!esSub)
          throw new ForbiddenException("No tenés permiso sobre esta rendición");
      }
    }
    ...
    const lineaRD = detalles.find((d: RendD) => d.U_RD_IdRD === idRD);
    if (!lineaRD)
      throw new NotFoundException(
        `Línea REND_D ${idRD} no encontrada en rendición ${idRendicion}`,
      );
    ...
    if (dto.lineas.some((l) => l.porcentaje <= 0)) {
      throw new BadRequestException(
        "Ninguna línea puede tener porcentaje 0 o negativo",
      );
    }

    if (dto.lineas.some((l) => !l.cuenta?.trim())) {
      throw new BadRequestException(
        "Todas las líneas deben tener una cuenta contable",
      );
    }
    ...
    const importe = lineaRD.U_RD_Importe ?? 0;

    // Reemplazar
    await this.repo.deleteByLinea(idRendicion, idRD, idUsuarioNum);
    await this.repo.insertLineas(
      idRendicion,
      idRD,
      idUsuarioNum,
      importe,
    ...
    this.logger.log(
      `PRCTJ guardado: Rend=${idRendicion} RD=${idRD} ${dto.lineas.length} líneas`,
    );
    return this.repo.findByLinea(idRendicion, idRD, idUsuarioNum);
  }
  ...
  async delete(
    ...
    if (role !== "ADMIN") {
      ...
      if (cabecera.U_Estado !== EstadoRendicion.ABIERTO) {
        throw new ForbiddenException(
          "Solo se puede eliminar la distribución en estado ABIERTO",
        );
      }
    }
