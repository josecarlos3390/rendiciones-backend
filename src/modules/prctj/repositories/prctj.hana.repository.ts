...
@Injectable()
export class PrctjHanaRepository {
  ...
  private get DB(): string     { return tbl(this.schema, 'REND_PRCTJ', this.dbType); }
  ...
  async insertLineas(
    ...
    for (const l of lineas) {
      ...
      await this.db.execute(
        ...
        [
          idRD, idRendicion, idUsuario,
          l.linea, l.porcentaje, monto,
          l.cuenta,       l.nomCuenta   ?? '',
          l.n1        ?? '', l.n2      ?? '', l.n3  ?? '', l.n4 ?? '', l.n5 ?? '',
          l.proyecto  ?? '',
          l.auxiliar1 ?? '', l.auxiliar2 ?? '', l.auxiliar3 ?? '', l.auxiliar4 ?? '',
        ],
      );
    }
    this.logger.log(`REND_PRCTJ: ${lineas.length} líneas insertadas para RD=${idRD} Rend=${idRendicion}`);
  }
