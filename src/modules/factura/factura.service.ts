...
export class FacturaService {
  ...
  private readonly apiUrl =
    "https://siatrest.impuestos.gob.bo/sre-sfe-shared-v2-rest/consulta/factura";
  ...
  async getFromSiat(rawUrl: string): Promise<FacturaSiatDto> {
    ...
    this.logger.log(
      `Consultando SIAT: NIT=${nit}, CUF=${cuf.substring(0, 12)}..., N°=${numero}`,
    );
    ...
    if (!response.ok) {
      throw new BadRequestException(
        `La API del SIAT respondió con HTTP ${response.status}`,
      );
    }
    ...
    if (data?.transaccion !== true) {
      throw new BadRequestException(
        "El SIAT indicó transacción fallida — verificá los datos del QR",
      );
    }
  ...
  private normalizarFecha(raw: string | null | undefined): string | null {
    ...
    try {
      ...
      // Intencionalmente vacío: cualquier error de parsing de fecha retorna null
    }
    return null;
  }

  private parseQrUrl(rawUrl: string): {
    cuf: string;
    nit: string;
    numero: string;
  } {
