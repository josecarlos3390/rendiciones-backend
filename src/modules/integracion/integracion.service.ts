...
import {
  ...
  INTEGRACION_REPOSITORY,
} from "./repositories/integracion.repository.interface";
...
import {
  PRCTJ_REPOSITORY,
  IPrctjRepository,
} from "../prctj/repositories/prctj.repository.interface";
import { RendPrctj } from "../prctj/interfaces/prctj.interface";
import {
  IRendCmpRepository,
  REND_CMP_REPOSITORY,
  SapFieldMapping,
} from "../rend-cmp/repositories/rend-cmp.repository.interface";
...
export class IntegracionService {
  ...
  constructor(
    ...
    @Inject(PRCTJ_REPOSITORY)
    private readonly prctjRepo: IPrctjRepository,
  ...
  async countPendientes(
    ...
    if (isAdmin || sinAprobador) {
      ...
      const count = await this.repo.countPendientesByAprobador(
        loginAprob,
        cascada,
      );
      return { count };
  ...
  async sincronizar(
    ...
    this.logger.log(
      `Sincronización ${idRendicion}: ${detalles.length} detalles obtenidos`,
    );
    if (detalles.length > 0) {
      this.logger.debug(
        `Primer detalle: U_RD_IdRD=${detalles[0].U_RD_IdRD}, U_RD_Cuenta=${detalles[0].U_RD_Cuenta}, U_RD_Importe=${detalles[0].U_RD_Importe}`,
      );
    }
    ...
    detalles.forEach((d, idx) => {
      ...
      if ((d.U_MontoIVA ?? 0) > 0 && !d.U_CuentaIVA?.trim())
        erroresCuentas.push(
          `${label}: IVA ${d.U_MontoIVA} sin cuenta contable`,
        );
      ...
      if ((d.U_MontoIUE ?? 0) > 0 && !d.U_CuentaIUE?.trim())
        erroresCuentas.push(
          `${label}: IUE ${d.U_MontoIUE} sin cuenta contable`,
        );

      if ((d.U_MontoRCIVA ?? 0) > 0 && !d.U_CuentaRCIVA?.trim())
        erroresCuentas.push(
          `${label}: RC-IVA ${d.U_MontoRCIVA} sin cuenta contable`,
        );
    });
    ...
    if (this.isOffline) {
      ...
      for (const d of detalles) {
        const dist = await this.prctjRepo.findByLinea(
          idRendicion,
          d.U_RD_IdRD,
          idUsuarioNum,
        );
      ...
      this.logger.log(
        `[OFFLINE] Rendición ${idRendicion} marcada como sincronizada`,
      );
      return {
        success: true,
        nroDocERP,
    ...
    if (!dto.sapUser?.trim() || !dto.sapPassword?.trim()) {
      throw new BadRequestException(
        "Credenciales SAP requeridas (sapUser y sapPassword)",
      );
    }
    ...
    try {
      ...
      const bolivianosEs = this.config
        .get<string>("app.bolivianosEs", "LOCAL")
        .toUpperCase();
      ...
      if (bolivianosEs === "SISTEMA") {
        ...
        this.logger.log(
          `Obteniendo tipo de cambio USD para fecha ${fechaCabecera}`,
        );
        tasaCambio = await this.sapSl.obtenerTasaCambio(
          session,
          "USD",
          fechaCabecera,
        );
      ...
      try {
        ...
        this.logger.debug(
          `Mapeo de campos SAP cargado: ${JSON.stringify(fieldMapping)}`,
        );
      } catch (err: unknown) {
        this.logger.warn(
          `Error cargando mapeo de campos desde REND_CMP: ${err instanceof Error ? err.message : String(err)}. Usando valores por defecto.`,
        );
      ...
      const payload = this.sapSl.buildJournalPayload(
        rend,
        detalles,
        distribucionesMap,
        tasaCambio,
      ...
      const nroDocERP = await this.sapSl.crearAsientoPreliminar(
        session,
        payload,
      );
      ...
      this.logger.log(
        `Rendición ${idRendicion} sincronizada — Doc. SAP: ${nroDocERP}`,
      );
      return {
        success: true,
        nroDocERP,
      ...
      const mensaje =
        err instanceof Error
          ? err.message
          : "Error desconocido al sincronizar con SAP";
      ...
      this.logger.error(
        `Error sincronizando rendición ${idRendicion}: ${mensaje}`,
      );
      return {
        success: false,
        mensaje,
