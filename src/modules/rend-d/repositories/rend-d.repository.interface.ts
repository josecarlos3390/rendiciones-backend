...
export interface IRendDRepository {
  ...
  findOne(idRendicion: number, idRD: number, idUsuario: number): Promise<RendD | null>;
  create(idRendicion: number, idUsuario: number, dto: CreateRendDDto): Promise<RendD | null>;
  update(idRendicion: number, idRD: number, idUsuario: number, dto: UpdateRendDDto): Promise<{ affected: number }>;
  remove(idRendicion: number, idRD: number, idUsuario: number): Promise<{ affected: number }>;
}