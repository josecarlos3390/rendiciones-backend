...
export class RendDHanaRepository implements IRendDRepository {
  ...
  private get DB_CTA(): string {
    return tbl(this.schema, "REND_CTA", this.dbType);
  }
  private get DB_PRCTJ(): string {
  ...
  private _normalizeEmpty(val: unknown): string | null {
  ...
  private _num(row: Record<string, any>, col: string, fallback = 0): number {
  ...
  private _numNull(row: Record<string, any>, col: string): number | null {
    const v = this.db.col(row, col);
  ...
  private normalize(row: Record<string, any>): RendD {
    return {
      ...
      U_RD_Cuenta: this._str(row, "U_RD_Cuenta", null) || null,
      U_RD_NombreCuenta: this._str(row, "U_RD_NombreCuenta", null) || null,
      ...
      U_RD_N1: this._str(row, "U_RD_N1", null) || null,
      U_RD_N2: this._str(row, "U_RD_N2", null) || null,
      U_RD_N3: this._str(row, "U_RD_N3", null) || null,
      U_RD_N4: this._str(row, "U_RD_N4", null) || null,
      U_RD_N5: this._str(row, "U_RD_N5", null) || null,
      U_RD_Proyecto: this._str(row, "U_RD_Proyecto", null) || null,
      ...
      U_RD_Partida: this._str(row, "U_RD_Partida", null) || null,
      ...
      U_RD_NumDocumento: this._str(row, "U_RD_NumDocumento", null) || null,
      U_RD_NroAutor: this._str(row, "U_RD_NroAutor", null) || null,
      U_RD_Ctrl: this._str(row, "U_RD_Ctrl", null) || null,
      U_RD_NIT: this._str(row, "U_RD_NIT"),
      U_RD_CodProv: this._str(row, "U_RD_CodProv", null) || null,
      U_RD_Prov: this._str(row, "U_RD_Prov", null) || null,
      ...
      U_CuentaIVA: this._str(row, "U_CuentaIVA", null) || null,
      U_CuentaIT: this._str(row, "U_CuentaIT", null) || null,
      U_CuentaIUE: this._str(row, "U_CuentaIUE", null) || null,
      U_CuentaRCIVA: this._str(row, "U_CuentaRCIVA", null) || null,
      ...
      U_CUF: this._str(row, "U_CUF", null) || null,
  ...
  async create(
    ...
    return mutex.runExclusive(async () => {
      ...
      const codProvLimpio = dto.codProv?.startsWith("PL")
        ? null
        : (dto.codProv ?? null);
      ...
      this.logger.log(
        `REND_D creado: ID ${newId} — rendición ${idRendicion} — usuario ${idUsuario}`,
      );
  ...
  async update(
    ...
    const nombreCuentaConcatenado =
      dto.cuenta && dto.nombreCuenta !== undefined
        ? `${dto.cuenta}-${dto.nombreCuenta}`
        : dto.nombreCuenta;
  ...
  async remove(
    ...
    return this.db.transaction(async (tx) => {
      // 1) Eliminar distribuciones porcentuales asociadas (cascade delete manual)
      await tx.execute(
        `DELETE FROM ${this.DB_PRCTJ} WHERE "PRCT_RD_RM_IDREND" = ? AND "PRCT_RD_IDRD" = ? AND "PRCT_RD_IDUSER" = ?`,
        [idRendicion, idRD, idUsuario],
      );
      // 2) Eliminar la línea de detalle
  ...
  async findHistorialClasificaciones(
    idUsuario: number,
    limit = 20,
  ): Promise<
    Array<{
  ...
  > {
    const rows = await this.db.query(
      `SELECT
         d."U_RD_Concepto" AS concepto,
         COALESCE(NULLIF(d."U_RD_NombreCuenta",''), d."U_RD_Cuenta") AS cuenta,
         d."U_RD_N1"       AS norma,
         d."U_RD_Proyecto" AS proyecto
       FROM ${this.DB} d
       INNER JOIN ${this.DB_M} m ON m."U_IdRendicion" = d."U_RD_RM_IdRendicion"
       WHERE d."U_RD_IdUsuario" = ?
         AND d."U_RD_Concepto" IS NOT NULL
         AND TRIM(d."U_RD_Concepto") <> ''
         AND d."U_RD_Cuenta" IS NOT NULL
         AND TRIM(d."U_RD_Cuenta") <> ''
       ORDER BY m."U_FechaCreacion" DESC, d."U_RD_IdRD" DESC
       LIMIT ?`,
      [idUsuario, limit],
    );
    ...
    return rows.map((r) => ({
      ...
      norma: r.norma ? String(r.norma) : null,
      proyecto: r.proyecto ? String(r.proyecto) : null,
    }));
  }
}
