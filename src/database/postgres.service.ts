  ...
  implements IDatabaseService, OnModuleInit, OnModuleDestroy
{
  ...
  private async createPool(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `PostgreSQL no disponible al iniciar: ${err instanceof Error ? err.message : String(err)}. Se reintentará en cada query.`,
      );
    }
  }
  ...
  private async destroyPool(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        "Error al cerrar el pool PostgreSQL",
  ...
  async query<T = any>(rawSql: string, params: unknown[] = []): Promise<T[]> {
  ...
  async execute(rawSql: string, params: unknown[] = []): Promise<number> {
    const { sql, values } = this.convertPlaceholders(rawSql, params);
  ...
  private buildTransactionProxy(client: PoolClient): IDatabaseService {
    return {
      ...
      transaction: () =>
        Promise.reject(
          new Error("No se pueden anidar transacciones en PostgreSQL"),
        ),
